...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	var totalMdls []Middleware
	cur := root

	// handle root pth mdls
	if len(cur.mdls) != 0 {
		totalMdls = append(totalMdls, cur.mdls...)
	}
	if cur.starChild != nil {
		totalMdls = append(totalMdls, cur.starChild.mdls...)
	}

	for i, s := range segs {
		var ok bool
		var segMdls []Middleware

		cur, _, ok = cur.childOf(s)
		if !ok {
			return nil
		}

		// add mdl when path is static match
		if len(cur.mdls) != 0 && cur.path != "*" {
			segMdls = append(segMdls, cur.mdls...)
		}

		// add mdl when path is star match
		// if the path is the last one, it should not match star_child again
		if cur.starChild != nil && i+1 != len(segs) {
			segMdls = append(segMdls, cur.starChild.mdls...)

			// handle middle star case, e.g. a/*/c
			if i+2 < len(segs) {
				tempCur, _, ok := cur.starChild.childOf(segs[i+2])
				if ok {
					segMdls = append(segMdls, tempCur.mdls...)
				}
			}
		}

		totalMdls = append(totalMdls, segMdls...)
	}

	cur.matchedMdls = totalMdls
